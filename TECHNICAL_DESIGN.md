...
| 指标 | 目标值 | 说明 |
|------|--------|------|
| API 平均响应时间 (P50) | < 200ms | 不含 SSE 流式接口；CUD 类操作直接返回 |
| API 响应时间 (P99) | < 1s | 慢查询需走缓存或异步化 |
| SSE 首次事件延迟 | < 1.5s | 从提交生成到第一个进度事件到达前端 |
| 行程查询接口 (GET) | < 100ms P99 | 关联数据通过 JOIN 或 JSON 字段一次加载 |
| 酒店选择/确认接口 | < 200ms P99 | 单行更新，无跨服务调用 |
| LLM 首 Token 等待 | < 3s | 受模型 API 网络延迟制约，非系统瓶颈 |

#### QPS 容量规划

| 场景 | 预估 DAU | 单用户日均请求 | QPS 峰值 | 说明 |
|------|----------|---------------|----------|------|
| 内测/开发阶段 | 100 | 10 | ~0.05 | 单实例无压力 |
| 公测 MVP | 1,000 | 15 | ~0.5 | 单实例即可承载 |
| 早期增长期 | 10,000 | 20 | ~5 | 2 实例 + Redis 缓存有效削峰 |
| 规模运营期 | 100,000 | 25 | ~50 | 4-6 实例 + 读写分离 + CDN |

**计算说明**（以早期增长期为例）：
- DAU 10,000，人均日请求 20 → 日均请求 200,000
- 小时峰值系数 3x → 峰值小时 60,000 → 峰值 QPS ≈ 16.7
- 接口间不均匀系数（部分热点接口集中）→ 综合峰值 ≈ 50 QPS

#### 资源规划

| 组件 | 内测/开发 | 公测 MVP | 早期增长 | 规模运营 |
|------|-----------|----------|----------|----------|
| API 实例 | 1 台 (2C4G) | 1 台 (4C8G) | 2 台 (4C8G) | 4-6 台 (4C8G) |
| Worker 实例 | 1 台 (2C4G) | 1 台 (4C8G) | 2 台 (4C8G) | 3-4 台 (8C16G) |
| MySQL | 共享实例 | 2C4G 独享 | 4C8G 独享 | 8C16G + 只读副本 |
| Redis | 共享实例 | 2G 独享 | 4G 独享 | 8G 集群 |
| ChromaDB | 嵌入 API 实例 | 独立 2C4G | 独立 4C8G | 独立 4C8G |

#### 容量瓶颈与应对

| 瓶颈点 | 风险等级 | 应对策略 |
|--------|----------|----------|
| LLM API 调用（外部依赖） | ⚠️ 高 | 增加 Worker 并发数；引入请求排队优先级；降级方案：返回缓存的历史行程 |
| MySQL 单表查询 | ⚠️ 中 | 索引覆盖；JSON 字段避免 OLAP 查询；必要时分表 |
| Redis 内存 | ✅ 低 | 设 TTL；大 Value 拆分；预估 4G 可承载百万级缓存条目 |
| ChromaDB 向量检索 | ✅ 低 | 当前 POI 数量级（千级）可全内存加载；万级以上使用 ANN 索引 |
...
```
src/
├── api/              # 路由层：薄，只做参数校验和响应包装
│   └── v1/
│       ├── router.py          # 路由聚合（新增模块在此注册）
│       ├── itineraries.py     # 行程模块路由
│       ├── cities.py          # 城市模块路由
│       ├── pois.py            # POI 模块路由
│       └── users.py           # 用户模块路由（新增）
│
├── services/         # 业务逻辑层：可独立测试
│   ├── queue/        # 队列服务（接口统一，实现可替换）
│   ├── stream/       # SSE 流服务
│   ├── cache/        # 缓存服务（Redis 实现，可替换为内存/Memcached）
│   ├── llm/          # LLM 调用（多提供商抽象）
│   └── rag/          # RAG 检索服务
│
├── agent/            # Agent 层：工具注册式扩展
│   ├── tools/        # 工具模块（新增工具 = 新建文件 + 注册）
│   └── workflow/     # Agent 工作流（可替换为自定义 StateGraph）
│
└── models/           # 数据模型层：ORM + Pydantic
    ├── tables.py     # 表定义（新增表在此定义）
    └── schemas.py    # Pydantic 模式（新增接口模式在此定义）
```

**模块间依赖方向**：`api → services → models`，禁止反向依赖。Agent 层可依赖 services 和 models，但不依赖 api。
...
```
            ...
            ┌──────────┐ ┌──────────┐ ┌──────────┐
            │  Worker  │ │  Worker  │ │  Worker  │  ← 通过 Redis List 消费任务
            │  Inst 1  │ │  Inst 2  │ │  Inst N  │     竞争消费模式
            └──────────┘ └──────────┘ └──────────┘
```
...
| 维度 | 方案 |
|------|------|
| **API 无状态** | Session 信息存在 Redis；请求不绑定特定实例；任意实例可处理任意请求 |
| **Worker 竞争消费** | 多个 Worker 消费同一 Redis List；`BRPOP` 竞争获取任务；天然负载均衡 |
| **数据库连接池** | 每个实例维护独立连接池；按 `max_connections / 实例数` 配置上限 |
| **SSE 亲和性** | SSE 连接绑定到实例内存（EventSource 长连接）；断开时通过轮询兜底恢复 |
| **静态资源** | 前端资源通过 Next.js 或 CDN 分发；不依赖后端实例本地存储 |

**新增一个功能模块的步骤**：

1. **数据层**：在 `tables.py` 新增 ORM 模型 → `alembic revision --autogenerate` 生成迁移
2. **Schema**：在 `schemas.py` 新增 Pydantic 请求/响应模型
3. **服务层**：在 `services/` 下新增业务逻辑模块（或扩展现有模块）
4. **路由层**：在 `api/v1/` 下新增路由文件 → 在 `router.py` 注册
5. **Agent 工具**（如需要）：在 `agent/tools/` 下新增工具文件 → 在 `__init__.py` 导入

每个步骤增量式、不影响已有功能，且无需修改核心框架代码。
